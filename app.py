...
st.caption("AI-driven regression score prediction, pass/fail classification, deterministic risk analysis, and TreeSHAP explainability.")
...
if predict_score_clicked or predict_class_clicked:
    ...
    # Display key prediction metrics and deterministic risk rating
    score_col, status_col, probability_col, confidence_col, risk_col = st.columns(5)
    score_col.metric("Predicted Score", f"{result.predicted_score:.2f}%")
    status_col.metric("Pass/Fail Result", result.pass_fail)
    probability_col.metric("Pass Probability", f"{result.pass_probability * 100:.1f}%")
    confidence_col.metric("Confidence Score", f"{result.confidence_score * 100:.1f}%")

    risk_badge = {"LOW": "LOW (🟢)", "MODERATE": "MODERATE (🟡)", "HIGH": "HIGH (🔴)"}.get(result.risk_level, result.risk_level)
    risk_col.metric("Risk Level", risk_badge)

    st.caption(f"**Risk Assessment:** {result.risk_description} (Risk Index: {result.risk_index})")

    left, right = st.columns([1.2, 1])
    with left:
        st.subheader("TreeSHAP Local Feature Attributions")
        if result.contributions.empty:
            ...
            figure = px.bar(
                ...
                title=f"Top Local SHAP Values (Base Value = {result.base_value:.2f})",
            )
            figure.update_layout(height=480, yaxis_title="", xaxis_title="SHAP Value (Impact on Prediction)")
    ...
    with right:
        ...
        if result.model_comparison:
            ...
            comp_df = pd.DataFrame(result.model_comparison).T
