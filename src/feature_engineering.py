...
def add_engineered_features(
    data: pd.DataFrame,
    median_previous_grade: float | None = None,
) -> pd.DataFrame:
    ...
    median_previous_grade : float | None, optional
        Population median for previous_grade from the training dataset.
        During single-student inference, passing the precomputed training median
        prevents the single sample from setting its own median (which would force
        grade_trend to 0.0). If None and len(data) > 1, the sample median is used.
    """
    df = data.copy()
    ...
    if median_previous_grade is not None:
        benchmark_median = float(median_previous_grade)
    elif "previous_grade" in df.columns and len(df) > 1:
        benchmark_median = float(df["previous_grade"].median())
    else:
        benchmark_median = 68.0

    df["grade_trend"] = df["previous_grade"] - benchmark_median
    df["risk_index"] = (
