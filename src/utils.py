...
def load_uci_dataset_strict() -> pd.DataFrame:
    """Load the real UCI Student Performance dataset strictly without synthetic fallback.

    Raises RuntimeError if real UCI data is unavailable.
    """
    ensure_directories()
    raw_mat_path = DATA_RAW_DIR / "student-mat.csv"
    raw_standard_path = DATA_RAW_DIR / "student_performance.csv"

    if raw_mat_path.exists():
        data = standardize_uci_dataset(raw_mat_path)
    ...
    try:
        ...
        raise RuntimeError(
            f"STRICT_UCI_DATA_REQUIRED: Failed to load real UCI Student Performance dataset: {exc}. "
            "Synthetic fallback is strictly prohibited for the final benchmark."
        ) from exc
